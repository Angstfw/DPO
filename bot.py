...
def get_photo_path(message):
    try:
        ...
        response = requests.post('http://localhost:5000/upload_photo', files={'photo': file.content}, data = {"photo_path": f"{name}.jpg"})
        ...
        response = requests.post('http://localhost:5000/save_data', data=data)
...
def handle_text(message):
    object_name = message.text
    response = requests.get(f'http://localhost:5000/get_data?object_name={object_name}')
    ...
    try:
        ...
        for photo in photo_path_data:
            url = 'http://localhost:5000/get_photo'
